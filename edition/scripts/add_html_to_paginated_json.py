...
the type 2 segs from the lit reading we end up witht he original reading plus
the line breaks.
...
class DisplayTextGenerator(object):
    ...
    def process_start_cb(self, element):
        ...
        if self.column_number == 0:
            ...
            if main and main_id != self.current_main_column: 
                ...
                self.current_main_column = main_id
            elif main and main_id == self.current_main_column: #this is end of subcolumn
                column_html.append('</div>')
                self.subcolumn = False
                ...
                if self.subcolumn:
                    ...
                    column_html.append('<br class="clear"/>')
                column_html.append('<div class="subcolumn col-md-%d">' % (12/len(self.column_structure[main_id])))  
                self.subcolumn = True
    ...
    def process_start_am(self, element):
        ...
        if element.text:
            return '<span class="abbreviation_marker hoverover" data-tooltip-content="#amex-%d">%s' % (self.amex_pos, element.text.replace('⁊', 'τ')) 
        else:
            return '<span class="abbreviation_marker hoverover" data-tooltip-content="#amex-%d">' % self.amex_pos
    ...
    def process_end_ex(self, element):
        ...
        if self.expanded:
            ...
            tooltip = '<div class="tooltip_templates"><span class="expansion_details" id="amex-%d">%s expands to %s</span></div>' % (self.amex_pos, ''.join(self.am_text), ''.join(self.ex_text))        
            self.am_text = []
            self.ex_text = []
            self.amex_pos += 1
            return tooltip
